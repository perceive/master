...
class purchase_sample_type(osv.Model):

    _name = 'purchase.sample.type'
...
class purchase_order(osv.Model):
    ...
    _columns = {
        'sample_line_id': fields.one2many('purchase.sample.order.line', 'order_id', 'Sample Order Lines'),
        }

class purchase_sample_order_line(osv.Model):

    _name = 'purchase.sample.order.line'

    _columns = {
        "name": fields.char("Name", size=64),
        "sample_type": fields.many2one('purchase.sample.type', 'Types', select=True),
        'order_id': fields.many2one('purchase.order', 'Order Reference', ondelete='cascade', select=True, readonly=True),
        ...
        'partner_id': fields.related('order_id', 'partner_id', type="many2one", relation='res.partner', store=True, string='Supplier')
    }
    ...
    _constraints = [
        (_check_follow_up, 'Error!\nThe follow up date of purchase order must greater then current date.', ['follow_up'])
    ]
    ...
    def scheduler_manage_activity(self, cr, uid, domain=[], model_name=False, context=None):
        ...
        template_id = 'email_template_purchase_sample'
        ...
        for activity_id in self.browse(cr, uid, ids, context=context):
            if activity_id.alarm_id:
                ...
                if time_now2 == scheduled_time_date and actual_time == time_now:
                    self.do_mail(cr, uid, [activity_id.id], template_id, 'purchase.sample.order.line', context=context)
    ...
    def do_mail(self, cr, uid, id, template, model, context=None):
        ...
        try:
            template_id = ir_model_data.get_object_reference(cr, uid, 'purchase', template)[1]
        except ValueError:
            template_id = False
        for sample in self.browse(cr, uid, id, context=context):
            group_obj = self.pool.get('res.groups')
            user_obj = self.pool.get('res.users')
            category_id = self.pool.get('ir.module.category').search(cr, uid, [('name','=','Purchases')])
    ...
    def run_scheduler(self, cr, uid, context=None):
        ...
        self.scheduler_manage_activity(cr, uid, domain=domain, model_name='purchase.sample.order.line', context=context)
        return True